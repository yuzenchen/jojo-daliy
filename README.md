...
熱資料（App 的 JSON 包）只保留最近 800 筆；伺服器另有 `logs_archive` 表
**永久保存每一筆**（寫入時自動同步、刪除也同步），SQLite 撐百萬筆沒問題：

- 月曆點任何月份／日期都是查歸檔（`GET /api/history?fromTs&toTs`），不受 800 筆限制。
- 匯出 Google 試算表也是完整歸檔。
- Pi 上有 cron 每天 03:15 自動匯出到試算表（`crontab -l` 可查）——試算表同時
  是異地備份，SD 卡掛了紀錄也在雲端。
